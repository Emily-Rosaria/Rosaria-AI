    ...
    if (!data || !data.documents || !data.documents.has(title)) {
      Docs.find({name: title}, (err,docs)=>{
        if (err || !docs || docs.length == 0) {
          return button.reply.send(`No saved note found with that name in the database. You'll need to create your own with the \`$newnote "${title}"\` command for this button to work.`,true);
        }
