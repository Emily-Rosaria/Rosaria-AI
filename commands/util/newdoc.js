...
module.exports = {
    name: 'newnote', // The name of the command
    aliases: ['newdoc','adddoc','addnote'],
    description: 'Adds a text "document" that the bot can repost whenever. Overwrites any note the user has saved if the name is identical. Pings and mentions are removed, so use plaintext if you wish to reference another user.', // The description of the command (for help text)
    perms: 'basic',
    allowDM: true,
    group: 'notes',
    args: 2,
    ...
    async execute(message, args) {
      ...
      if (data && data.documents && data.documents.has(title)) {
        ...
        message.reply("An old note with that name was overwritten with the new text provided! Use the `$note "+title+"` command to have the bot say it.");
      } else {
        message.reply("Your note has been created! Use the `$note "+title+"` command to have the bot say it.");
      }
    },
};
